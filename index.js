...
function jalaliDate() {
  ...
  const month = String(j.jm)
    .padStart(2, '0');

  const day = String(j.jd)
    .padStart(2, '0');
...
function buildTitle(prefix = '') {
  ...
  return `${clockEmoji()} ${prefix} ${tehranTimeFancy()}`
    .trim();
...
🕒 Time : ${tehranTimeFancy()}
📅 Date : ${jalaliDate()}
🌍 Zone : ${fancyEnglish('Asia/Tehran')}
...
bot.on('channel_post', async (ctx) => {
  ...
  if (ctx.channelPost?.new_chat_title) {
    ...
    try {
      ...
      console.log(
        `🗑 Deleted Service Message | ${ctx.chat.id}`
      );
